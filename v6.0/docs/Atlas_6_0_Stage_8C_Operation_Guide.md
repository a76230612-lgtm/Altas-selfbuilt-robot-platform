# Atlas 6.0 Stage 8C Practical Avoidance V1 — 操作手册
...
完整固件：`Atlas_6_0_Stage_8C_Practical_Avoidance_V1.ino`
...
1. 打开`Atlas_6_0_Stage_8C_Practical_Avoidance_V1.ino`；
...
4. 确认显示`state : FORCE_STOP`。

每个距离只测一次。三项都正确就继续。

### 2.4 SENSOR_INVALID_STOP
...
1. 把软纸箱放在US-100前方约45 cm；
2. 确保小车右侧至少有40 cm空地；
3. 左侧也不要放人、手、宠物或易碎物；
4. 输入`status`，确认`CAUTION`；
5. 依次输入：
...
- US-100能区分CLEAR、CAUTION、FORCE_STOP和无效数据；
- 左右`check`均通过且方向正确；
- `drive 80 150`通过；
...
Stage 8C Practical Obstacle Avoidance: PASSED
```
