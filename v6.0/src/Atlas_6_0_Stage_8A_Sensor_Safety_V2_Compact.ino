...
// ============================================================
// Atlas 6.0 Stage 8A Sensor Safety V2 Compact
// ESP32 DEVKIT_C + L298N + 双编码器 + US-100
//
// 目标：
// 1. 完整保留 Stage 6C V4 的电机、编码器、目标停车和安全功能。
// 2. 加入 US-100 前向距离安全门：无效数据立即停车，近障碍禁止前进。
// 3. 小空间无动力测试阈值：25 cm 以下 FORCE_STOP；25–40 cm CAUTION；40 cm 以上 CLEAR。
// 4. 从 CAUTION 恢复到 CLEAR 时使用 45 cm 释放线，避免临界点反复切换。
// 5. CAUTION 状态把向前方向的 PWM 限制到 100；恢复后不会自动提速。
// 6. 安全状态恢复后必须重新 ARM，绝不自动继续之前的运动。
// 7. 本 Stage 8A 文件强制关闭真实电机输出，仅用于小空间无动力软件整合测试。
// 8. 进入 Stage 8B 或任何真实移动测试前，必须恢复正式阈值 25 / 50 / 55 cm。
//
...
constexpr float CAUTION_ENTER_CM = 40.0f;
constexpr float CLEAR_RELEASE_CM = 45.0f;
...
void printConfig() {
  ...
  Serial.println("version         : Atlas 8A sensor safety v2 compact");
  Serial.print("motor outputs   : ");
  ...
  Serial.println("US-100 thresholds: <25 stop, 25-40 caution, >40 clear");
  Serial.println("clear hysteresis: caution -> clear at 45 cm");
...
void printHelp() {
  ...
  Serial.println();
  Serial.println("Stage 8A compact software test (motor outputs are disabled):");
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println("Atlas 8A sensor safety v2 compact is ready");
