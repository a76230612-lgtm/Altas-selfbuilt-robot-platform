...
// ============================================================
// Atlas 6.0 Stage 7D Continuous Distance V6
//
// 这一版完整保留已经验收通过的 Stage 7C：
//   USB / Wi-Fi TCP 共用命令处理、ARM、心跳、断线停车、编码器检查。
//
// 只替换 FDS 的落地距离控制核心：
//   V5 的 45 ms 短脉冲反复启停在地面负载下几乎不能克服静摩擦；
//   V6 改为 PWM 80 连续驱动、50 ms 编码器同步、每轮独立提前制动。
// 旧 FD、定时运动、SYNC 与 CHECK 命令保持原逻辑，便于回退和对照。
// ============================================================
...
// -------------------- Stage 7D V6 连续距离控制参数 --------------------
// 落地实测已经证明 f 80 500 能稳定前进，而 V5 的 45 ms 启停脉冲不能稳定起步。
// 因此 FDS 全程保持最低可靠 PWM 80，只给落后轮最多 +8 PWM 的小幅补偿。
constexpr int CONTINUOUS_FDS_BASE_PWM = 80;
constexpr unsigned long CONTINUOUS_FDS_CONTROL_INTERVAL_MS = 50;
constexpr unsigned long CONTINUOUS_FDS_REPORT_INTERVAL_MS = 200;
constexpr float CONTINUOUS_FDS_KP_PWM_PER_EQUIV_PULSE = 0.15f;
constexpr float CONTINUOUS_FDS_DEADBAND_EQUIV_PULSES = 3.0f;
constexpr int CONTINUOUS_FDS_MAX_BOOST_PWM = 8;
constexpr int CONTINUOUS_FDS_MAX_PWM_STEP = 1;

// 每个轮子到达“目标 - 提前量”后独立动态制动。提前量按目标的 7% 计算，
// 且限制在 3~10 pulses；即使制动后完全没有滑行，也仍在 10% 精度门槛内。
constexpr float CONTINUOUS_FDS_BRAKE_LEAD_FRACTION = 0.07f;
constexpr int32_t CONTINUOUS_FDS_MIN_BRAKE_LEAD_PULSES = 3;
constexpr int32_t CONTINUOUS_FDS_MAX_BRAKE_LEAD_PULSES = 10;

constexpr int32_t CONTINUOUS_FDS_REVERSE_LIMIT_PULSES = 20;
constexpr unsigned long CONTINUOUS_FDS_STALL_TIMEOUT_MS = 800;
constexpr unsigned long CONTINUOUS_FDS_MIN_TOTAL_TIMEOUT_MS = 5000;
constexpr unsigned long CONTINUOUS_FDS_MAX_TOTAL_TIMEOUT_MS = 20000;
constexpr unsigned long CONTINUOUS_FDS_TIMEOUT_PER_TARGET_PULSE_MS = 20;
constexpr unsigned long CONTINUOUS_FDS_TIMEOUT_MARGIN_MS = 3000;
...
bool continuousDistanceEnabled = false;

int continuousLeftPwm = 0;
int continuousRightPwm = 0;
int32_t continuousLeftBrakeLead = 0;
int32_t continuousRightBrakeLead = 0;
unsigned long continuousNextControlMs = 0;
unsigned long continuousNextReportMs = 0;
...
void clearMotionState() {
  ...
  continuousDistanceEnabled = false;

  continuousLeftPwm = 0;
  continuousRightPwm = 0;
  continuousLeftBrakeLead = 0;
  continuousRightBrakeLead = 0;
  continuousNextControlMs = 0;
  continuousNextReportMs = 0;
...
const char* friendlyStopReason(const char* reason) {
  ...
  if (strcmp(reason, "CONTINUOUS_STALL_LEFT") == 0) {
    return "Left forward pulse count did not increase for 800 ms during V6 continuous FDS.";
  }
  if (strcmp(reason, "CONTINUOUS_STALL_RIGHT") == 0) {
    return "Right forward pulse count did not increase for 800 ms during V6 continuous FDS.";
  }
  if (strcmp(reason, "CONTINUOUS_REVERSE_LEFT") == 0) {
    return "Left forward count became negative beyond the V6 continuous-distance limit.";
  }
  if (strcmp(reason, "CONTINUOUS_REVERSE_RIGHT") == 0) {
    return "Right forward count became negative beyond the V6 continuous-distance limit.";
  }
  ...
  if (strcmp(reason, "TARGET_TIMEOUT") == 0) {
    return "The target was not completed within the controller's distance-scaled timeout.";
  }
...
void printTargetResult(
  ...
  if (continuousDistanceEnabled) {
    console.print("V6 base PWM     : ");
    console.println(CONTINUOUS_FDS_BASE_PWM);
    console.print("brake lead      : left ");
    console.print(continuousLeftBrakeLead);
    console.print(", right ");
    console.println(continuousRightBrakeLead);
    console.print("count at brake  : left ");
    console.print(leftCountAtBrake);
    console.print(", right ");
    console.println(rightCountAtBrake);
  }
...
void finishMotion(const char* reason, bool disarmSystem) {
  ...
  const bool finishedContinuousDistance = continuousDistanceEnabled;
  const unsigned long finishedElapsedMs =
      targetStartMs > 0 ? millis() - targetStartMs : 0;
  ...
  if (wasMoving) {
    ...
    if (finishedMode == MotionMode::TARGET) {
      ...
      if (finishedCommand == "FDS") {
        console.println();
        if (finishedContinuousDistance) {
          console.println("[FDS V6 continuous-controller report]");
          console.println("drive structure : continuous PWM; no repeated start/stop cycles");
          console.print("base / max PWM  : ");
          console.print(CONTINUOUS_FDS_BASE_PWM);
          console.print(" / ");
          console.println(
              CONTINUOUS_FDS_BASE_PWM + CONTINUOUS_FDS_MAX_BOOST_PWM);
          console.print("control interval: ");
          console.print(CONTINUOUS_FDS_CONTROL_INTERVAL_MS);
          console.println(" ms");
          console.print("elapsed         : ");
          console.print(finishedElapsedMs);
          console.println(" ms before final report");
          console.println("ESP32 restart   : record YES only if the startup banner reappeared");
        } else {
          console.println("[Legacy FDS pulse-approach report]");
  ...
  if (strcmp(reportedReason, "STALL_LEFT") == 0 ||
      ...
      strcmp(reportedReason, "CONTINUOUS_STALL_LEFT") == 0 ||
      strcmp(reportedReason, "CONTINUOUS_STALL_RIGHT") == 0 ||
      strcmp(reportedReason, "CONTINUOUS_REVERSE_LEFT") == 0 ||
      strcmp(reportedReason, "CONTINUOUS_REVERSE_RIGHT") == 0) {
...
void printStatus() {
  ...
  console.println(" ms (drops only after an ESP32 restart)");
...
void printConfig() {
  ...
  console.println("version         : Atlas 7D continuous distance V6 over Stage 7C");
  ...
  console.println(" ms");
  console.print("V6 timeout range: ");
  console.print(CONTINUOUS_FDS_MIN_TOTAL_TIMEOUT_MS);
  console.print(" to ");
  console.print(CONTINUOUS_FDS_MAX_TOTAL_TIMEOUT_MS);
  ...
  console.println("new FDS         : V6 continuous PWM 80 ground-distance controller");
  console.print("V6 interval     : ");
  console.print(CONTINUOUS_FDS_CONTROL_INTERVAL_MS);
  console.println(" ms");
  console.print("V6 deadband     : ");
  console.print(CONTINUOUS_FDS_DEADBAND_EQUIV_PULSES, 1);
  console.println(" equivalent pulses");
  console.print("V6 max boost    : +");
  console.print(CONTINUOUS_FDS_MAX_BOOST_PWM);
  console.println(" PWM to lagging wheel only");
  console.print("V6 stall timeout: ");
  console.print(CONTINUOUS_FDS_STALL_TIMEOUT_MS);
  console.println(" ms");
  console.print("brake lead      : ");
  console.print(CONTINUOUS_FDS_MIN_BRAKE_LEAD_PULSES);
  console.print(" to ");
  console.print(CONTINUOUS_FDS_MAX_BRAKE_LEAD_PULSES);
  console.println(" pulses, scaled by target");
...
void printHelp() {
  ...
  console.println("  arm  then  fds 5   (Stage 7D V6 continuous ground controller)");
  console.println();
...
int32_t calculateContinuousBrakeLeadPulses(int32_t targetPulses) {
  int32_t brakeLead = static_cast<int32_t>(lroundf(
      static_cast<float>(targetPulses) *
      CONTINUOUS_FDS_BRAKE_LEAD_FRACTION));

  if (brakeLead < CONTINUOUS_FDS_MIN_BRAKE_LEAD_PULSES) {
    brakeLead = CONTINUOUS_FDS_MIN_BRAKE_LEAD_PULSES;
  }
  if (brakeLead > CONTINUOUS_FDS_MAX_BRAKE_LEAD_PULSES) {
    brakeLead = CONTINUOUS_FDS_MAX_BRAKE_LEAD_PULSES;
  }
  if (brakeLead >= targetPulses) {
    brakeLead = targetPulses > 1 ? targetPulses - 1 : 0;
  }
  return brakeLead;
}

unsigned long calculateContinuousTargetTimeoutMs(
    int32_t requestedLeftTarget,
    int32_t requestedRightTarget) {
  ...
  const int32_t maximumTarget =
      requestedLeftTarget > requestedRightTarget
          ...
          : requestedRightTarget;
  unsigned long calculatedTimeout =
      static_cast<unsigned long>(maximumTarget) *
      CONTINUOUS_FDS_TIMEOUT_PER_TARGET_PULSE_MS +
      CONTINUOUS_FDS_TIMEOUT_MARGIN_MS;

  if (calculatedTimeout < CONTINUOUS_FDS_MIN_TOTAL_TIMEOUT_MS) {
    calculatedTimeout = CONTINUOUS_FDS_MIN_TOTAL_TIMEOUT_MS;
  }
  if (calculatedTimeout > CONTINUOUS_FDS_MAX_TOTAL_TIMEOUT_MS) {
    calculatedTimeout = CONTINUOUS_FDS_MAX_TOTAL_TIMEOUT_MS;
  }
  return calculatedTimeout;
}
...
void startTargetMotion(
  ...
  continuousDistanceEnabled =
      command == "FDS" && leftTargetActive && rightTargetActive;
  targetSyncEnabled = continuousDistanceEnabled;
  targetPulseApproachEnabled = false;

  continuousLeftPwm = continuousDistanceEnabled
      ? CONTINUOUS_FDS_BASE_PWM
      : 0;
  continuousRightPwm = continuousDistanceEnabled
      ? CONTINUOUS_FDS_BASE_PWM
      : 0;
  continuousLeftBrakeLead = continuousDistanceEnabled
      ? calculateContinuousBrakeLeadPulses(requestedLeftTarget)
      : 0;
  continuousRightBrakeLead = continuousDistanceEnabled
      ? calculateContinuousBrakeLeadPulses(requestedRightTarget)
      : 0;
  continuousNextControlMs =
      now + CONTINUOUS_FDS_CONTROL_INTERVAL_MS;
  continuousNextReportMs =
      now + CONTINUOUS_FDS_REPORT_INTERVAL_MS;
  ...
  activeTargetTimeoutMs = continuousDistanceEnabled
      ? calculateContinuousTargetTimeoutMs(
          requestedLeftTarget,
          requestedRightTarget)
  ...
  const int initialLeftPwm = continuousDistanceEnabled
      ? CONTINUOUS_FDS_BASE_PWM
      : (targetPulseApproachEnabled ? 0 : LEFT_TARGET_PWM);
  const int initialRightPwm = continuousDistanceEnabled
      ? CONTINUOUS_FDS_BASE_PWM
      : (targetPulseApproachEnabled ? 0 : RIGHT_TARGET_PWM);
  ...
  console.print("mode            : ");
  if (continuousDistanceEnabled) {
    console.println("Stage 7D V6 continuous ground-distance control");
  } else if (targetPulseApproachEnabled) {
    console.println("legacy adaptive pulse approach");
  } else {
    console.println("legacy target motion");
  }
  console.print("command         : ");
  console.println(displayCommand);
  ...
  if (continuousDistanceEnabled) {
    console.println("controller      : continuous PWM 80 with encoder synchronization");
    console.println("stop rule       : each wheel brakes independently before its target");
    console.print("control interval: ");
    console.print(CONTINUOUS_FDS_CONTROL_INTERVAL_MS);
    console.println(" ms");
    console.print("PWM range       : ");
    console.print(CONTINUOUS_FDS_BASE_PWM);
    console.print(" to ");
    console.println(
        CONTINUOUS_FDS_BASE_PWM + CONTINUOUS_FDS_MAX_BOOST_PWM);
    console.print("left brake at   : ");
    console.print(leftTargetPulses - continuousLeftBrakeLead);
    console.print(" pulses (lead ");
    console.print(continuousLeftBrakeLead);
    console.println(")");
    console.print("right brake at  : ");
    console.print(rightTargetPulses - continuousRightBrakeLead);
    console.print(" pulses (lead ");
    console.print(continuousRightBrakeLead);
    console.println(")");
    console.print("total timeout   : ");
    console.print(activeTargetTimeoutMs);
    console.println(" ms");
...
int32_t continuousStopThreshold(int32_t target, int32_t brakeLead) {
  const int32_t threshold = target - brakeLead;
  return threshold > 0 ? threshold : 1;
}

void beginContinuousLeftBrake(int32_t physicalLeft, unsigned long now) {
  ...
  console.println();
  console.println("[V6 wheel brake started]");
  console.println("wheel           : left");
  console.print("count at brake  : ");
  console.print(physicalLeft);
  console.print(" / ");
  console.println(leftTargetPulses);
  console.print("planned lead    : ");
  console.print(continuousLeftBrakeLead);
  console.println(" pulses");
}

void beginContinuousRightBrake(int32_t physicalRight, unsigned long now) {
...
void updateContinuousDistancePwm(
    int32_t physicalLeft,
    int32_t physicalRight,
    unsigned long now) {

  if (static_cast<int32_t>(now - continuousNextControlMs) < 0) {
    return;
  }

  do {
    continuousNextControlMs += CONTINUOUS_FDS_CONTROL_INTERVAL_MS;
  } while (static_cast<int32_t>(now - continuousNextControlMs) >= 0);
  ...
  int targetLeftPwm = CONTINUOUS_FDS_BASE_PWM;
  int targetRightPwm = CONTINUOUS_FDS_BASE_PWM;

  // 只有两轮都仍在行驶时才比较同步误差。某轮已经制动后，另一轮只需
  // 以基准 PWM 完成自己剩余的少量距离，避免停止轮被任何控制写入释放制动。
  if (!leftTargetDone && !rightTargetDone) {
    if (errorEquivalent > CONTINUOUS_FDS_DEADBAND_EQUIV_PULSES) {
      int boost = static_cast<int>(lroundf(
          (errorEquivalent - CONTINUOUS_FDS_DEADBAND_EQUIV_PULSES) *
          CONTINUOUS_FDS_KP_PWM_PER_EQUIV_PULSE));
      if (boost < 1) {
        boost = 1;
      }
      if (boost > CONTINUOUS_FDS_MAX_BOOST_PWM) {
        boost = CONTINUOUS_FDS_MAX_BOOST_PWM;
      }
      targetRightPwm += boost;

    } else if (errorEquivalent < -CONTINUOUS_FDS_DEADBAND_EQUIV_PULSES) {
      int boost = static_cast<int>(lroundf(
          (-errorEquivalent - CONTINUOUS_FDS_DEADBAND_EQUIV_PULSES) *
          CONTINUOUS_FDS_KP_PWM_PER_EQUIV_PULSE));
      if (boost < 1) {
  ...
  continuousLeftPwm = movePwmToward(
      continuousLeftPwm,
      targetLeftPwm,
      CONTINUOUS_FDS_MAX_PWM_STEP);
  continuousRightPwm = movePwmToward(
      continuousRightPwm,
      targetRightPwm,
      CONTINUOUS_FDS_MAX_PWM_STEP);

  if (!leftBrakeActive) {
    setLeftMotor(leftTargetDone ? 0 : continuousLeftPwm);
  }
  if (!rightBrakeActive) {
    setRightMotor(rightTargetDone ? 0 : continuousRightPwm);
  }

  if (static_cast<int32_t>(now - continuousNextReportMs) >= 0) {
    continuousNextReportMs = now + CONTINUOUS_FDS_REPORT_INTERVAL_MS;
    console.print("FDS V6 t=");
    console.print(now - targetStartMs);
    console.print(" ms; L=");
    console.print(physicalLeft);
    console.print("/");
    console.print(leftTargetPulses);
    ...
    console.print(errorEquivalent, 2);
    console.print("; PWM=");
    console.print(leftTargetDone ? 0 : continuousLeftPwm);
    console.print("/");
    console.println(rightTargetDone ? 0 : continuousRightPwm);
  }
}

void monitorContinuousDistanceMotion() {
  if (!moving || motionMode != MotionMode::TARGET ||
      !continuousDistanceEnabled) {
    return;
  }
  ...
  // 制动保持时间结束后，明确将该轮输出关闭。PWM同步函数在制动期间不会
  // 写入该轮，因此不会意外提前释放动态制动。
  if (leftBrakeActive &&
  ...
  if (!leftTargetDone &&
      physicalLeft < -CONTINUOUS_FDS_REVERSE_LIMIT_PULSES) {
    finishMotion("CONTINUOUS_REVERSE_LEFT", true);
    return;
  }
  if (!rightTargetDone &&
      physicalRight < -CONTINUOUS_FDS_REVERSE_LIMIT_PULSES) {
    finishMotion("CONTINUOUS_REVERSE_RIGHT", true);
    return;
  }
  ...
  if (!leftTargetDone &&
      physicalLeft >= continuousStopThreshold(
          leftTargetPulses,
          continuousLeftBrakeLead)) {
    beginContinuousLeftBrake(physicalLeft, now);
  }
  if (!rightTargetDone &&
      physicalRight >= continuousStopThreshold(
          rightTargetPulses,
          continuousRightBrakeLead)) {
    beginContinuousRightBrake(physicalRight, now);
  }
  ...
  if (!leftTargetDone) {
    ...
    if (now - leftLastProgressMs >= CONTINUOUS_FDS_STALL_TIMEOUT_MS) {
      finishMotion("CONTINUOUS_STALL_LEFT", true);
      return;
    }
  }
  ...
  if (!rightTargetDone) {
    ...
    if (now - rightLastProgressMs >= CONTINUOUS_FDS_STALL_TIMEOUT_MS) {
      finishMotion("CONTINUOUS_STALL_RIGHT", true);
      return;
    }
  }
  ...
  updateContinuousDistancePwm(physicalLeft, physicalRight, now);
}
...
void monitorTargetMotion() {
  ...
  if (continuousDistanceEnabled) {
    monitorContinuousDistanceMotion();
    return;
  }
...
void processCommand(String line, CommandSource source) {
  ...
  if (command == "FD" || command == "FDS") {
    ...
    console.println(command == "FDS"
        ? "Stage 7D V6 continuous ground-distance control"
...
void setup() {
  ...
  console.println();
  console.println("========================================");
  console.println("Atlas 7D continuous distance V6 is ready");
  console.println("wireless base   : Stage 7C safety and TCP bridge preserved");
  console.println("FDS controller  : continuous PWM 80 with per-wheel braking");
