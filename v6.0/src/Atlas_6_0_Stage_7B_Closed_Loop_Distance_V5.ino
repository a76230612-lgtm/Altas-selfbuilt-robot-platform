  ...
  if (wasMoving) {
    ...
    if (finishedMode == MotionMode::TARGET) {
      ...
      if (finishedCommand == "FDS") {
        Serial.println();
        Serial.println("[FDS phase report]");
        Serial.print("final cycle     : ");
        Serial.print(approachCycleCount);
        Serial.print(" / dynamic limit ");
        Serial.println(activeApproachMaxCycles);
        Serial.print("accuracy recovery entered : ");
        Serial.println(accuracyRecoveryEntered ? "yes" : "no");
        Serial.println("recovery meaning: fine-distance correction; NOT an ESP32 reset");
        Serial.print("program-held leading wheel: ");
        if (leftLeadingWheelHeld && rightLeadingWheelHeld) {
          Serial.println("both at different cycles");
        ...
        Serial.println("held meaning    : intentionally paused by software; NOT a wheel fault");
        Serial.println("ESP32 restart   : record YES only if the startup banner reappeared");
      }
...
void printConfig() {
  ...
  Serial.println("version         : Atlas 7B closed-loop distance v5");
  ...
  Serial.println(" ms");
  Serial.print("timeout range   : ");
  Serial.print(TARGET_MIN_TOTAL_TIMEOUT_MS);
  Serial.print(" to ");
  Serial.print(TARGET_MAX_TOTAL_TIMEOUT_MS);
  Serial.println(" ms; selected from FDS target size");
  ...
  Serial.print(APPROACH_MIN_MAX_CYCLES);
  Serial.print(" to ");
  Serial.print(APPROACH_ABSOLUTE_MAX_CYCLES);
  Serial.println("; selected from FDS target pulses");
...
void printHelp() {
  ...
  Serial.println("  arm  then  fds 5   (Stage 7B V5 dynamic-limit pulse approach)");
  Serial.println();
...
void startTargetMotion(
  ...
  if (targetPulseApproachEnabled) {
    ...
    Serial.print(activeTargetTimeoutMs);
    Serial.println(" ms");
  }
}
...
bool startNextApproachCycle(
  ...
  if (approachCycleCount > APPROACH_NORMAL_CYCLES) {
    ...
    if (approachCycleCount == APPROACH_NORMAL_CYCLES + 1) {
      Serial.println("phase meaning   : fine-distance correction; NOT an ESP32 reset");
    }
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println("Atlas 7B closed-loop distance v5 is ready");
