...
def main():
    parser = argparse.ArgumentParser(
        description="Atlas EDGE model final evaluator with explicit model path"
    )
    parser.add_argument("--model", required=True, help="Full path to best.pt")
    parser.add_argument(
        "--name",
        default="EDGE_Final_Evaluation",
        help="Output folder name beside this script",
    )
    ...
    print("=" * 82)
    print("Atlas 6.0 - EDGE Model Final Evaluation")
    print("=" * 82)
    ...
    csv_path = output_root / "edge_final_audit.csv"
    ...
    per_image_acc = correct / len(rows)
    strict_pass = (
        top1 >= PASS_TOP1
    ...
    print(f"Per-image accuracy            : {per_image_acc:.4f}")
