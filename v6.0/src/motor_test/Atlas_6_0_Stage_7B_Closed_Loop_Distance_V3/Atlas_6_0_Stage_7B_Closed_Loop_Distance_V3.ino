...
// ============================================================
// Atlas 6.0 Stage 7B Closed-Loop Distance V3
...
// 10. FDS 只有在每轮最终误差不超过 10%，且左右最终距离差不超过 5% 时
//     才显示 passed。V2 仍保留为回退版本，不要覆盖。
//
...
constexpr unsigned long TARGET_TOTAL_TIMEOUT_MS = 20000;
...
constexpr uint16_t APPROACH_MAX_CYCLES = 60;
...
const char* friendlyStopReason(const char* reason) {
  ...
  if (strcmp(reason, "TARGET_TIMEOUT") == 0) {
    return "The target was not completed within 20 seconds.";
  }
...
void printConfig() {
  ...
  Serial.println("version         : Atlas 7B closed-loop distance v3");
...
void printHelp() {
  ...
  Serial.println("  arm  then  fds 5   (Stage 7B V3 pulse-approach distance)");
  Serial.println();
...
void startTargetMotion(
  ...
  Serial.println(targetPulseApproachEnabled
      ? "Stage 7B V3 adaptive pulse-approach target motion"
...
void processCommand(String line) {
  ...
  if (command == "FD" || command == "FDS") {
    ...
    Serial.println(command == "FDS"
        ? "Stage 7B V3 adaptive pulse-approach distance"
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println("Atlas 7B closed-loop distance v3 is ready");
