...
// ============================================================
// Atlas 6.0 Stage 7B Closed-Loop Distance V2
...
// 8. V2 在每个轮子到达目标时启用 100 ms L298N 动态制动，随后再关断输出，
//    解决 V1 轮子离地时从 148 pulses 滑行到约 278 / 272 pulses 的问题。
// 9. V2 把“安全完成”和“最终距离精度”分开判断：FDS 只有在每个轮子的
//    最终误差不超过 10%，且左右最终距离差不超过 5% 时才显示 passed。
//
...
// Stage 7B V2 定距停车：到达目标后，L298N 使电机两端短时处于同电位，
// 利用电机反电动势进行动态制动。100 ms 后立即恢复为高阻关断状态。
constexpr int TARGET_BRAKE_PWM = 255;
constexpr unsigned long TARGET_BRAKE_HOLD_MS = 100;
...
void finishMotion(const char* reason, bool disarmSystem) {
  ...
  if (strcmp(reportedReason, "STALL_LEFT") == 0 ||
    ...
    Serial.println("next            : stop longer FDS tests; record final pulses and brake current");
...
void printConfig() {
  ...
  Serial.println("version         : Atlas 7B closed-loop distance v2");
  ...
  Serial.println("new FDS         : synchronized 80 PWM target-distance with dynamic brake");
...
void printHelp() {
  ...
  Serial.println("  arm  then  fds 5   (Stage 7B synchronized distance)");
  Serial.println();
...
void startTargetMotion(
  ...
  const int initialLeftPwm = targetSyncEnabled
      ? syncLeftPwm
      : LEFT_TARGET_PWM;
  const int initialRightPwm = targetSyncEnabled
      ? syncRightPwm
      : RIGHT_TARGET_PWM;
  ...
  Serial.println("[Motion started]");
  Serial.print("mode            : ");
  Serial.println(targetSyncEnabled
      ? "Stage 7B synchronized target motion"
  ...
  if (targetSyncEnabled) {
    Serial.println("controller      : Stage 7 V2 boost-lagging-wheel synchronization");
    Serial.println("stop rule       : each wheel brakes independently at its own target");
...
void processCommand(String line) {
  ...
  if (command == "FD" || command == "FDS") {
    ...
    Serial.println(command == "FDS"
        ? "Stage 7B synchronized low-speed distance"
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println("Atlas 7B closed-loop distance v2 is ready");
