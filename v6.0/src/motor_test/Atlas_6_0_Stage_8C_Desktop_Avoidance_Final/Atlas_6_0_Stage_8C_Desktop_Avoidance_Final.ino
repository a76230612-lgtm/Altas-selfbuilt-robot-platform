...
// ============================================================
// Atlas 6.0 Stage 8C Desktop Avoidance V2
...
//   - Four-level US-100 safety state for an 11 cm x 11 cm desktop robot.
//   - One bounded right-first / left-fallback avoidance manoeuvre.
...
// Desktop distance model (distance is measured from the US-100 face):
//   <= 8 cm       FORCE_STOP: emergency stop; do not start a motion.
//   8 to 18 cm    CAUTION: stop forward motion and begin a bounded scan/turn.
//   18 to 30 cm   WATCH: valid approach zone; avoidance may continue slowly.
//   >= 30 cm      CLEAR: normal start zone.
// Hysteresis prevents rapid state switching close to a boundary.
constexpr float FORCE_STOP_ENTER_CM = 8.0f;
constexpr float FORCE_STOP_RELEASE_CM = 10.0f;
constexpr float CAUTION_ENTER_CM = 18.0f;
constexpr float CAUTION_RELEASE_CM = 22.0f;
constexpr float CLEAR_ENTER_CM = 30.0f;
constexpr float CLEAR_HOLD_CM = 28.0f;
...
const char* sensorStateName(SensorState state) {
  ...
  if (state == SensorState::WATCH) return "WATCH";
  return "CLEAR";
}
...
SensorState classifyFilteredDistance(float filteredCm,
                                     SensorState previousState) {
  if (filteredCm <= FORCE_STOP_ENTER_CM) {
    return SensorState::FORCE_STOP;
  }

  // A FORCE_STOP needs extra clearance before it can be released.
  if (previousState == SensorState::FORCE_STOP &&
      filteredCm <= FORCE_STOP_RELEASE_CM) {
    return SensorState::FORCE_STOP;
  }

  // CAUTION is the active avoidance band. Keep it until 22 cm so that
  // centimetre-scale noise cannot repeatedly start and cancel a turn.
  if (filteredCm <= CAUTION_ENTER_CM ||
      (previousState == SensorState::CAUTION &&
       filteredCm < CAUTION_RELEASE_CM)) {
    return SensorState::CAUTION;
  }

  // Once CLEAR, hold it down to 28 cm. A new/non-CLEAR reading needs
  // 30 cm before it first enters CLEAR.
  if (filteredCm >= CLEAR_ENTER_CM ||
      (previousState == SensorState::CLEAR &&
       filteredCm >= CLEAR_HOLD_CM)) {
    return SensorState::CLEAR;
  }

  return SensorState::WATCH;
}
...
bool sampleUs100(bool forceSample) {
  ...
  sensorState = classifyFilteredDistance(
      filteredDistanceCm(),
      sensorState);
...
const char* friendlyReason(const char* reason) {
  ...
  if (strcmp(reason, "OBSTACLE_WATCH_STOP") == 0)
    return "Obstacle entered the WATCH range during manual timed drive.";
...
bool forwardSensorAllowsStart() {
  if (sensorState != SensorState::CLEAR) {
    ...
    console.println("next            : restore a valid CLEAR distance of at least 30 cm, then arm again");
    return false;
  }
  return true;
}
...
void monitorDriveAssist() {
  ...
  if (sensorState == SensorState::WATCH) {
    finishMotion("OBSTACLE_WATCH_STOP");
    return;
  }
...
void startAvoidance(unsigned long sessionDurationMs) {
  ...
  if (sensorState == SensorState::INVALID_STOP ||
    ...
    console.println("next            : restore a valid distance above 10 cm");
    return;
  }
  ...
  if (sensorState == SensorState::CLEAR ||
      sensorState == SensorState::WATCH) {
    beginAvoidForward();
  } else {
    avoidObstacleCycles = 1;
    beginAvoidWait(AvoidState::WAIT_BEFORE_RIGHT, AVOID_STOP_SETTLE_MS);
    console.println("initial state   : CAUTION; preparing bounded right scan");
  }
}
...
void printConfig() {
  ...
  console.print("; WATCH <");
  console.print(CLEAR_ENTER_CM, 0);
  console.print("; CLEAR >=");
  console.print(CLEAR_ENTER_CM, 0);
  console.println(" cm");
  console.print("state hysteresis: FORCE release >");
  console.print(FORCE_STOP_RELEASE_CM, 0);
  console.print("; CAUTION release >=");
  console.print(CAUTION_RELEASE_CM, 0);
  console.print("; CLEAR hold >=");
  console.print(CLEAR_HOLD_CM, 0);
  console.println(" cm");
...
void printHelp() {
  ...
  console.println("  WATCH approaches slowly; CAUTION starts the scan/turn");
  console.println();
...
void setup() {
  ...
  console.println("build           : Desktop Avoidance V2");
