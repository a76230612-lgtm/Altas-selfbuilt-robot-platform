/*
  Atlas 6.0 - Integrated Release Candidate V3.3 DIRECTION FIXED
...
void serviceAuto(){
  ...
  if(rangeC==RG_CLEAR){
    if(motion!=FORWARD){if(motion==STOPPED)driveWithStartupBoost(drivePwm,drivePwm);else drive(drivePwm,drivePwm);motion=FORWARD;sendLine("AUTO,ACTION,FORWARD");}
    return;
  }
...
void setup(){
  ...
  Serial.println();
  Serial.println("================================================");
  Serial.println("ATLAS 6.0 INTEGRATED RELEASE CANDIDATE V3.3 DIRECTION FIXED");
  Serial.println("================================================");
