...
// ============================================================
// Atlas 6.0 Stage 8B-1 Suspended Integration V1
...
// 7. 本 Stage 8B-1 文件启用真实电机输出，但只允许在轮子悬空、台式限流电源下测试。
// 8. 本文件尚未通过实体验收前，禁止落地行驶，禁止使用高放电倍率移动电池。
//
// 当前已锁定的编码器映射（信号组保持交换状态）：
//   物理右轮编码器 -> GPIO18 / GPIO19 -> 原始计数 RAW_18_19
//   物理左轮编码器 -> GPIO25 / GPIO26 -> 原始计数 RAW_25_26
//
// Stage 8B-1 保留 V4 的编码器前进符号自动学习。每次重启后必须先执行：
//   arm -> check left
//   arm -> check right
...
// Stage 8B-1 启用真实电机输出。实体测试必须轮子悬空并使用台式限流电源。
constexpr bool MOTOR_OUTPUTS_ENABLED = true;
...
void applyOneMotor(
  ...
  // 保留软件输出总开关；Stage 8B-1 中该值为 true。
...
void printStatus() {
  ...
  Serial.println(MOTOR_OUTPUTS_ENABLED ? "ENABLED (suspended test only)" : "DISABLED");
...
void printConfig() {
  ...
  Serial.println("version         : Atlas 8B-1 suspended integration v1");
  Serial.print("motor outputs   : ");
...
void printHelp() {
  ...
  Serial.println();
  Serial.println("Stage 8B-1 suspended motor test (real outputs are enabled):");
  Serial.println("  wheels must remain lifted; use a current-limited bench supply");
  Serial.println("  first run arm + check left, then arm + check right");
  Serial.println();
...
void startEncoderCheck(CheckWheel wheel) {
  if (!MOTOR_OUTPUTS_ENABLED) {
    ...
    Serial.println("reason          : motor outputs are disabled by the build configuration");
    Serial.println("next            : upload the verified Stage 8B-1 build before motor testing");
    return;
  }
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println("Atlas 8B-1 suspended integration v1 is ready");
  Serial.println("motors          : stopped");
  Serial.println("motor outputs   : ENABLED - WHEELS MUST STAY LIFTED");
  Serial.println("safety          : disarmed");
